...
public abstract class AbstractCommandExecutor {
    ...
    public TaskResponse runByExecPath(String execCommand,String execPath) throws IOException, InterruptedException {
        TaskResponse result = new TaskResponse();
//        int taskInstanceId = taskRequest.getTaskInstanceId();
//        if (null == TaskExecutionContextCacheManager.getByTaskInstanceId(taskInstanceId)) {
//            result.setExitStatusCode(EXIT_CODE_KILL);
...
//        }
        ...
        String commandFilePath = buildCommandExecPath(execPath);
...
//            ProcessUtils.kill(taskRequest);
    ...
    public TaskResponse run(String execCommand,String childTaskId) throws IOException, InterruptedException {
        TaskResponse result = new TaskResponse();
        int taskInstanceId = taskRequest.getTaskInstanceId();
//        if (null == TaskExecutionContextCacheManager.getByTaskInstanceId(taskInstanceId)) {
//            result.setExitStatusCode(EXIT_CODE_KILL);
...
//        }
        ...
        String commandFilePath = buildCommandFilePath(childTaskId);
    ...
    public void pkill(String processName) {
        if (processName != "" && process.isAlive()) {
            try {
                String cmd = String.format("pkill %s", processName);
                cmd = OSUtils.getSudoCmd(taskRequest.getTenantCode(), cmd);
                logger.info("pkill task:{}, process name:{}, cmd:{}", taskRequest.getTaskAppId(), processName, cmd);
    ...
    protected abstract String buildCommandFilePath(String childTaskId);

    protected abstract String buildCommandExecPath(String execPath);
